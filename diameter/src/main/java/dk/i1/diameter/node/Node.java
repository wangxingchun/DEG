...
public class Node {
	...
	public ConnectionKey findConnection(Peer peer)  {
		...
		synchronized(map_key_conn) {
		    ...
		    //System.out.println(conn.state.toString());
				if(conn.state!=Connection.State.ready)
					continue;
				
			//Modify by wangxingchun
				if(conn.peer!=null && peer!=null) {
					if(conn.peer.host().equals(peer.host_name())) {
						conn.peer.host(peer.host());
					}
				}
				
				//System.out.println("peer host:"+peer.host());
				//conn.peer.host("192.168.75.112");
				if(conn.peer!=null && conn.peer.equals(peer)) {
